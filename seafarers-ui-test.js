...
(async () => {
  ...
  try {
    ...
    async function recreateBeforeFirstSettlement(p, edition, ruleButtons) {
      ...
      if (edition !== "base") await p.locator("#sailingAcknowledge").click();
      assert.equal(await p.locator("dialog[open]").count(), 0, "The map introduction can be dismissed immediately");
    ...
    const baseContext = await browser.newContext({ viewport: { width: 390, height: 844 }, hasTouch: true });
    ...
    nextBase.game.phase = "robber"; nextBase.game.resumePhase = "main";
    await base.reload();
    await base.locator('#game[data-phase="robber"]').waitFor();
    const robberTile = E.legal(nextBase.game, 0).robber[0];
    await base.setViewportSize({ width: 390, height: 844 });
    await base.locator(`#board [data-tile="${robberTile}"]`).tap();
    await base.locator('#game[data-phase="main"]').waitFor();
    assert.equal(nextBase.game.board.robber, robberTile, "One mobile tap commits a robber destination");
    assert.equal(await base.locator("#selection").isVisible(), false);
    ...
    await page.locator(`#board [data-tile="${sea.id}"]`).tap();
    await page.locator("#board .built-ship.steal-target").first().waitFor();
    assert.equal(g.board.pirate, sea.id, "Tapping a destination moves the pirate immediately");
    assert.equal(await page.locator("#selection").isVisible(), false);
    ...
    for(const map of Maps.maps.slice(0, 8)) {
      ...
      assert.ok((await p.locator("#sailingTitle").textContent()).includes(map.english));
