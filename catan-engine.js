...
function makeBoard(rng = random, mapId = "base", layout = "default") {
  ...
  grid.forEach((hex, id) => {
    ...
    const tile = { id: tiles.length, q: hex.q, r: hex.r, x: hex.x, y: hex.y, resource: fixed?.resource ?? terrain[tiles.length], number: fixed?.number || 0, vertices: [], ...(fixed ? { row: fixed.row, col: fixed.col, frame: fixed.frame, setupAllowed: fixed.setupAllowed, ...(map.family === "desert" ? { region: fixed.region } : {}) } : {}) };
  ...
  if (map) {
    ...
    if (layout === "random") randomizeMap(map, tiles, edges, rng, shuffle);
    const islands = [];
    ...
    const portTypes = layout === "random" ? shuffle(map.ports.map((p) => p[3]), rng) : map.ports.map((p) => p[3]);
    const ports = map.ports.map(([row, col, angle], index) => {
    ...
    const robber = layout === "default" ? at(map.robber) : tiles.find((t) => t.resource === -1) || tiles.find((t) => t.number === 12);
    return { tiles, vertices, edges, ports, robber: robber.id, pirate: map.pirate ? at(map.pirate).id : -1, pirateStart, islands, mainIsland, mapId, layout, bounds: [Math.min(...xs) - margin, Math.min(...ys) - margin, Math.max(...xs) - Math.min(...xs) + margin * 2, Math.max(...ys) - Math.min(...ys) + margin * 2] };
  }
...
function createGame(names, rng = random, mapId = "base", layout = "default", preparedBoard = null) {
  requireRule(names.length === 3 || names.length === 4, "基础版需要 3–4 人 / Base game needs 3–4 players");
  const map = Maps.get(mapId);
  requireRule(!map || names.length === map.players, "人数不符合地图 / Wrong player count for this map");
  const g = {
...
function pieceCount(g, id, level) { return g.board.vertices.filter((v) => v.owner === id && v.level === level).length; }
...
function routeTerrain(g, e, kind) {
  return kind === "ship" ? g.board.pirate !== undefined && e.tiles.some((t) => g.board.tiles[t].resource <= -2) && !e.tiles.includes(g.board.pirate)
    : e.tiles.some((t) => g.board.tiles[t].resource >= -1);
}
function routeSites(g, id, kind, setup = false, ignore = -1) {
  if (routeCount(g, id, kind) - (ignore >= 0 ? 1 : 0) >= 15) return [];
  return g.board.edges.filter((e) => e.id !== ignore && e.owner < 0 && routeTerrain(g, e, kind) && (setup ? [e.a, e.b].includes(g.setupVertex) : [e.a, e.b].some((vId) => {
...
function movableShips(g, id) {
  if (!g.board.islands || g.shipMoved || g.phase !== "main") return [];
...
function score(g, id, hidden = true) {
  return g.board.vertices.filter((v) => v.owner === id).reduce((n, v) => n + v.level, 0)
    ...
    + (g.players[id].discovered?.length || 0) * 2
...
function updateAwards(g) {
  ...
  g.longest = award(g.roadLengths, g.longest, 5);
  g.largest = award(g.players.map((p) => p.knights), g.largest, 3);
}
function checkWin(g) {
...
function produce(g, number) {
  ...
  g.board.tiles.filter((t) => t.number === number && t.id !== g.board.robber && t.resource >= 0 && t.resource < 5).forEach((t) => {
  ...
  g.board.tiles.filter((t) => t.resource === 5 && t.number === number && t.id !== g.board.robber).forEach((t) => t.vertices.forEach((vId) => { const v = g.board.vertices[vId]; if (v.owner >= 0) gold[v.owner] += v.level; }));
...
function steal(g, id, victim, rng) {
...
function act(g, id, a, rng = random) {
  ...
  if (type === "gold") {
    ...
    if (!Object.keys(g.discard).length) g.phase = "robber";
    ...
    if (type === "settlement") {
      ...
      if (g.board.islands && Maps.get(g.mapId)?.bonusPoints) {
      ...
      if (setup) {
        ...
        if (g.setupStep >= g.players.length) {
          let gold = 0;
          v.tiles.forEach((t) => { const r = g.board.tiles[t].resource; if (r >= 0 && r < 5) take(g, p, r); else if (r === 5) gold++; });
      ...
      if (phase === "setupRoad") {
        ...
        if (g.setupStep === 2 * n) { g.phase = "roll"; g.turn = 1; g.current = 0; log(g, "初始建造完成 / Setup complete"); }
        else { g.current = g.setupStep < n ? g.setupStep : 2 * n - 1 - g.setupStep; g.phase = "setupSettlement"; }
      ...
      if (number === 7) {
        ...
        g.phase = Object.keys(g.discard).length ? "discard" : "robber";
      ...
      g.victims = [...new Set(owners)].filter((v) => v >= 0 && v !== id && sum(g.players[v].resources));
      log(g, `${p.name} ${type === "pirate" ? "移动海盗 / moves the pirate" : "移动强盗 / moves the robber"}`);
      if (g.victims.length === 1) steal(g, id, g.victims[0], rng);
      else g.phase = g.victims.length ? "steal" : g.resumePhase;
    } else if (type === "steal") {
      requireRule(g.phase === "steal", "现在不能偷取资源 / Cannot steal now"); steal(g, id, a.victim, rng);
      ...
      p.development.splice(index, 1); g.usedDevelopment.push(a.card); g.developmentPlayed = true; g.resumePhase = g.phase;
      ...
      if (a.card === "knight") { p.knights++; g.phase = "robber"; updateAwards(g); }
...
function publicGame(g, id) {
  return {
    ...
    players: g.players.map((p) => ({ id: p.id, name: p.name, resourceCount: sum(p.resources), developmentCount: p.development.length,
      ...
      settlements: pieceCount(g, p.id, 1), cities: pieceCount(g, p.id, 2), roads: routeCount(g, p.id, "road"), ships: routeCount(g, p.id, "ship"), islandPoints: (p.discovered?.length || 0) * 2,
    })),
...
function botBuildPlans(g, id) {
  ...
  return goals.map(([type, priority]) => {
    const cost = COST[type], missing = cost.reduce((n, amount, r) => n + Math.max(0, amount - hand[r]) * scarcity[r], 0);
...
module.exports = { RES, COST, LABEL, makeBoard, createGame, act, legal, publicGame, chooseBotAction, canRespondToTrade, longestRoad, updateAwards, score, produce, tradeRate, settlementSites, roadSites, shipSites, movableShips, citySites, sum, random };
