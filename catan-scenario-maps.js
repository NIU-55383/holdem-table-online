  ...
  const maps = [
    {
      ...
      rules: [
        ["初始与后续村庄只能建在主岛。外岛没有数字，不生产任何资源，也不能建村；外岛地形本身不提供奖励，只有海岸边的礼物标记可领取。", "Build all settlements on the mainland. Foreign islands have no number tokens, never produce resources and cannot be settled. Their terrain gives no reward; only the gifts on their coasts can be claimed."],
        ["领取方式：按通常连接规则，将自己的一艘船建造或移动到礼物标记所连接的那一条海岸边，系统立即领取该处的一份礼物。仅到达附近、停在相邻边或点击标记查看说明都不算领取。所有礼物由最先到达的玩家拿走一次，之后不补充。", "Claim a gift by legally building or moving your ship onto the exact coastal edge linked to its marker. Arriving nearby, occupying a neighboring edge or opening its description does not claim it. Each gift goes to the first player to reach it and is not replenished."],
        ["六个海港礼物：五个资源专门港（各一种，2:1）和一个通用港（3:1）。领取的是可搬回主岛的海港，不是资源卡，也不是船。领取后须立即放在自己沿海村庄或城市旁的合法海岸边，不得与现有港口重合或共用交叉点；没有合法位置则暂存，之后一有合法位置就必须放置。放好当回合即可使用。", "Six harbor gifts: five resource-specific 2:1 harbors and one generic 3:1 harbor. The gift is a harbor to relocate to the mainland, not resource cards or a ship. Place it immediately beside your own coastal settlement or city, without overlapping or sharing an intersection with another harbor. Keep it only while no legal site exists; place it as soon as one becomes available. It is usable on the turn placed."],
        ...
        ["另外有八枚各 1 分的胜利点标记，以及四张背面朝上的发展卡。领取发展卡不付购买费用，但遵守通常出牌时限：行动发展卡不能在领取当回合使用；胜利点卡立即计分。", "There are also eight 1-VP gifts and four face-down development cards. Gift cards cost nothing to collect but follow normal timing: action cards cannot be played on the turn collected; victory-point cards count immediately."],
    ...
    },
    {
      ...
      rules: [
        ["初始在两座大岛建三个村庄，第三轮从起始玩家顺时针进行；只按第三个村庄领取起始资源。中央四座小岛不能建村。", "Start with three settlements on the two main islands; the third round runs clockwise from the starting player. Take starting resources only from the third settlement. The four central islands cannot be settled."],
        ...
        ["至少连接一个村落后才可移动海盗；可从受害者偷一张资源或一匹布。强盗不能进入中央小岛。本剧本没有最长商路奖励。", "You may move the pirate only after connecting to a village, stealing either a resource or one cloth. The robber cannot enter central islands. Longest Trade Route is not awarded."],
