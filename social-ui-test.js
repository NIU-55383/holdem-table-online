...
(async () => {
  ...
  try {
    ...
    for (const kind of ["catan", "poker", "gomoku", "xiangqi", "richman"]) {
      ...
      for (const p of pages) await p.evaluate(() => document.querySelectorAll("dialog[open]").forEach((d) => d.close()));
      ...
      console.log(`${kind}: shared menu, mobile bounds, room delivery, animation expiry, own avatar editor passed`);
    ...
    assert.equal(await guest.locator("dialog[open]").count(), 0, "Joining a friend's Seafarers room does not force rules reading");
