...
布匹贸易中央岛屿：四岛统一使用荒漠外观，保留八个布匹村落及库存；不更改地图地形数据和布匹结算。规则注明中央岛屿不产资源，强盗不能封锁布匹村落。修复随机地图开局强盗误落中央沙漠的问题，改为只选择允许强盗的地块。更新棋盘渲染、规则、引擎及测试；外观刷新即可，开局修复需服务器部署更新后新开一局。完整包仍为 81 文件。

布匹贸易玩家表：最长商路列显示“不可用 / Not available”，不再显示路线长度；已建道路和船只数量保持不变。更新 `catan.js`、`catan.css` 与浏览器测试，其他地图保持原样，完整包仍为 81 文件。

布匹贸易胜利目标补全：`catan.js` 显示“自己的回合达 14 分”及“或有布村落仅剩 3 个：分高者胜，同分比布匹”，附简短英文。沿用现有特殊胜利条件排版，`catan-scenario-ui-test.js` 检查手机与桌面换行；结算规则不变。完整包仍为 81 文件。
...
| `social-data.js` | 共用互动表情、强盗与海盗皮肤白名单，必须上传 |
...
| `catan-maps.js` | 航海家十三张地图注册、分区和默认/随机双语规则 |
| `catan-scenario-maps.js` | 新增：后五个剧本的地图、特殊位置、奇观牌与随机约束 |
| `catan-scenarios.js` | 新增：部落礼物、布匹、海盗战斗、奇观及新世界规则，服务器必需 |
| `catan-scenario-ui.js` | 新增：高级剧本选择、海港放置和奇观操作界面 |
| `catan-scenario-art.svg` | 原创布匹、堡垒、战舰与五张奇观插画；布匹已重绘为珊瑚色布卷，与页面内嵌图案同步 |
...
- `catan-scenario-maps-test.js`：后五图的照片库存、特殊位置、固定区与随机生成约束。
- `catan-scenario-engine-test.js`：后五剧本的真实规则、非法操作、隐藏信息和完整机器人对局。
- `catan-scenario-network-test.js`：后五图的人数、房主托管权限、海港轮放、重连和新世界选项。
- `catan-scenario-ui-test.js`：可选浏览器测试，检查手机图标点选、布匹、战斗、奇观与海港。
- `social-skins-test.js`：强盗皮肤兼容、默认图案保持、内嵌资源与手机浏览器测试。
...
本次完整包共 81 个文件，包含此前各轮更新。`vendor` 是必须上传的运行目录，不能省略，也不要把里面的文件移动到根目录。提交后需要 Render 重新部署服务器，单独刷新旧服务器页面不会启用新地图、皮肤、头像互动、房间管理和动作音效。

## 本次更新

奇观牌费用与进度说明：更新 `catan-scenario-ui.js`、`catan.css` 和 `catan-scenario-ui-test.js`。卡面标明“建 1 个阶段，支付”，资源图标加名称，并说明每阶段都需重复支付相同费用；1–4 格显示“完成／下步／待建”和已完成数量，按钮明确支付资源建第几阶段，数字格不执行操作。桌面与手机均检查卡面文字、费用、进度与按钮无重叠；完整包仍为 81 文件。

奇观胜利提示：更新 `catan.html`、`catan.js`、`catan.css`、`catan-scenario-ui.js` 和 `catan-scenario-ui-test.js`。地图上方完整区分“建满 4 阶段”与“至少 10 分且奇观阶段数超过所有对手”，中英文分行，明确自己的回合满足任一项即可；海盗要塞显示同时满足的条件。特殊提示高度随内容增长，不再挤入地图，普通地图保持紧凑分数显示。完整包仍为 81 文件。

世外部落外观更新：更新 `catan-board.js`、`catan-scenario-ui.js`、`catan-scenario-maps.js`、`catan.css` 及 `catan-scenario-ui-test.js`。默认图和随机图的外岛统一使用现有沙漠贴图，岛上不显示文字；胜利点标记使用与发展卡相同的奖杯图标。地图介绍和点击说明保留领取、交易与放置规则，删除多余的澄清句。主岛、海港标记与游戏逻辑不变，点击说明不触发建设。完整上传包仍为 81 文件；本地刷新即可，线上需重新上传部署。

电脑版玩家信息布局：更新 `catan.js`、`catan.css` 和 `catan-scenario-ui-test.js`。宽度大于 740px 时，玩家表移到地图／回合提示下方，奇观牌上方；手机保持原顺序。保留同一个表格及头像互动，不复制玩家数据。刷新本地页面即可生效；线上上传并部署。完整包仍为 81 个文件。

新世界海港转初始建设提示：更新 `catan.html`、`catan.css`、`catan.js` 和 `catan-scenario-ui-test.js`。放完海港后显示一次确认提示，明确接下来放村庄，确认前拦截连续点击；地图上方同步显示当前初始建设阶段，手机横竖屏均可见。运行中的本地服务器无需重启，刷新页面即可；线上需上传并部署这些文件。完整包仍为 81 个文件。

新增世外部落、布匹贸易、海盗巢穴、卡坦奇观、新世界，航海家合计十三张地图。新增四个运行文件和五个测试文件，同时覆盖卡坦岛页面、样式、交互、棋盘、规则引擎、地图注册、随机执行器与服务器。默认／随机地图各按剧本约束；新世界开局先轮流放港，海盗巢穴只随机港口。页面内嵌新图案，不依赖远程图片。统一强盗称呼，默认外观不变，中间两款改为蒙面强盗。规则明确每人 5 村庄、4 城市、15 道路、15 船，以及标准发展卡 14/5/2/2/2 张与剧本例外。以下 72 文件条目为历史更新记录，本次请使用上方 81 文件完整包。

本次超时托管按新要求改为房主统一设置，默认关闭：覆盖 `room-control.js`、`game-ui.js`、`catan-server.js`、`richman-server.js`、`server.js`，同步共享测试与说明。五游戏仅房主可更改全员 120 秒托管开关，其他人只读；转让与补位沿用房间设置，最后 20 秒提醒。关闭后停止超时触发的托管，保留玩家主动托管。断线或离开本身不接管或弃牌。此项取代上一版的个人超时开关。

金矿提醒和建设上限说明：覆盖 `catan.html`、`catan.css`、`catan.js`、`catan-engine.js`、`catan-audio.js`，同步 `catan-test.js`、`catan-audio-test.js`、`catan-feedback-ui-test.js`。金矿当前领取者有金色提醒和专属提示音，服从静音、不会因重复状态或刷新重播；点满额的建设按钮显示原因：5 村庄、4 城市、15 道路、15 船。当时的 72 文件包现已并入上方 81 文件完整包；需重新部署并重新开房，仅刷新旧服务器不会生效。

五游戏移除真人／机器人统一双重确认：覆盖 `game-ui.js`、`game-ui.css`、`catan.js`、`online.js`、`richman.js`、`catan-server.js`、`server.js` 和 `richman-server.js`，同步 `room-control-test.js`、`room-control-ui-test.js`、`AGENTS.md`、`README.md` 和本清单。开局后通过房间管理可移除机器人，仍保留资产、暂停等待补位；等待室原有移除机器人入口也有两步确认，任一步可取消，并防止旧确认误删替换者。此次没有加入未获授权的商业 BGM，原有本地音效与开关保持不变。当时的 72 文件包现已并入上方完整包，需要重新部署服务。
...
五个游戏都支持双方确认转让房主、按号位顺时针自动接任、开局后移除玩家并暂停、真人或机器人补位。120 秒超时托管默认关闭，仅房主可为全员开启并在最后 20 秒提醒。被移出者仍可重新输入房间号加入有空位的房间，不是永久封禁；全部空位补齐后沿用原棋局与资产继续。
